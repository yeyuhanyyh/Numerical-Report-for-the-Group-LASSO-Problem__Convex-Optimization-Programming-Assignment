...
function [x,iter,out] = gl_FProxGD_primal(x0, A, b, mu, opts)
    ...
    grad_fx = @(z) A.'* (A * z - b); % 定义目标函数的梯度
    prox_hx = @(z,t) diag(max(0,1 - t * sqrt(sum(z.^2, 2)).^-1)) * z; % 定义近端操作
    ...
    for i = 1:1000
        ...
        theta = (iter - 2) / (iter + 1);
        y = x + theta * (x - xg); % 计算y
        z = y - tau * grad_fx(y); % 计算z
        xg = x;
        x = prox_hx(z, scale_mu * tau); % 更新x
        if rem(iter,100)==0
            scale = max(scale / 4, 1);
            scale_mu = scale * mu;
            if scale <= 1
    ...
    for i = 1:1000
        ...
        xg = x;
        x = prox_hx(z, scale_mu * tau); % 更新x
        new_obj = obj_fun(x, mu); % 计算新的目标函数值
        x(abs(x)<1e-5) = 0;
